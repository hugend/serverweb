...
<body>
	...
	<h4>==> 표현식으로 정의하는 데이터는 out.print()의 매개변수로 전달된다.
			따라서 표현식으로 정의할때는 ; 을 추가하지 않는다. ;을 추가하는 것은 print(값;)와 
			동일한 결과가 된다. 
			표현식은 자바에서 실행되는 값을 웹 페이지에 출력할 목적으로 사용하는 jsp 스크립트 요소이며
			사용가능한 타입은 제한적이다. 타입이 제한적인 이유는 서블릿으로 변환될때
			print메소드의 매개변수로 전달되므로...기본형, 스트링으로 변환될 수 있는 변수의 값,
	...
	<h3>2. 스크립트 릿 - 자바코드를 쓸 수 있는 스크립트요소</h3>
	<%
	//자바명령문을 정의할 수 있는 곳 ; 을 문장의 끝에 추가해야한다.
	// 기본으로 인식하는 패키지는 java.lang패키지밖에 없다. 다른 패키지를 사용하는 경우
	// import 해야한다. jsp에서 자동으로 만들어지는 지역변수를 사용할 수 있다.
